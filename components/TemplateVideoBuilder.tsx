...
export default function TemplateVideoBuilder({lang,onBack,onDone}:Props){
 ...
 return <section className="navWorkspace specializedPanel">
  <div className="workspaceCrumbs"><button data-action-id="mobi.components.templatevideobuilder.button.001" type="button" className="backKey" onClick={onBack}>← Back</button><button data-action-id="mobi.components.templatevideobuilder.button.002" type="button" className="crumbKey selected">{vi?"Tạo video":"Create Video"}</button><button data-action-id="mobi.components.templatevideobuilder.button.003" type="button" className="crumbKey selected tree5Crumb">{vi?"Từ mẫu":"From Template"}</button></div>
  ...
  <div className="endCommitRow"><button data-action-id="mobi.components.templatevideobuilder.button.004" type="button" className="secondaryEnd" onClick={onBack}>{vi?"Chọn lại":"Choose again"}</button><button data-action-id="mobi.components.templatevideobuilder.button.005" type="button" className="endCommit" disabled={!files.length} onClick={()=>setRendered(true)}>{rendered?(vi?"✓ Video sẵn sàng":"✓ Video ready"):(vi?"Tạo video":"Create video")}</button>{rendered&&<button data-action-id="mobi.components.templatevideobuilder.button.006" type="button" className="endCommit" onClick={onDone}>{vi?"Đưa vào Phát sóng · END":"Send to Broadcast · END"}</button>}</div>
 </section>
}
