...
export default function TemplateVideoBuilder({lang,onBack,onDone}:Props){
 ...
 return <section className="navWorkspace specializedPanel">
  ...
  <fieldset className="specialGrid" disabled={busy} style={{border:0,minWidth:0}}>
   ...
   <div className="templatePreview wide"><b>{vi?"Hoặc tìm ảnh mẫu bên ngoài":"Or find external sample images"}</b><div className="aiComposer"><input value={query} onChange={e=>setQuery(e.target.value)}/><button data-action-id="mobi.scene.assets.search" type="button" onClick={searchAssets}>{vi?"Tìm nguồn":"Search"}</button></div><div className="stickerGrid">{assets.map(asset=><button data-action-id={`mobi.scene.assets.select.${asset.id}`} type="button" key={asset.id} onClick={()=>setSelectedAsset(asset)} className={selectedAsset?.id===asset.id?"priority":""}><img src={asset.previewUrl} alt="" loading="lazy"/><small>{asset.license} · {asset.creator}</small></button>)}</div>{selectedAsset&&<small>{selectedAsset.attribution} · <a href={selectedAsset.sourceUrl} target="_blank" rel="noreferrer">Source/license evidence</a></small>}{status&&<div className="connectResult">{status}</div>}</div>
  ...
  <div className="endCommitRow"><button data-action-id="mobi.components.templatevideobuilder.button.004" type="button" className="secondaryEnd" onClick={()=>{abortRef.current?.abort();onBack()}}>{vi?"Chọn lại":"Choose again"}</button><button data-action-id="mobi.components.templatevideobuilder.button.005" data-runtime-ignore="true" type="button" className="endCommit" disabled={busy||!files.length} onClick={createVideo}>{rendered?(vi?"✓ Video đã tạo":"✓ Video created"):(vi?"Tạo video":"Create video")}</button>{rendered&&!output&&<button data-action-id="mobi.components.templatevideobuilder.button.006" type="button" className="endCommit" onClick={onDone}>{vi?"Đưa vào Phát sóng":"Send to Broadcast"}</button>}</div>
 </section>
}
