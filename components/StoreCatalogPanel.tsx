  ...
  return (
    <section className="navWorkspace contentSurface storeCatalog" data-runtime-area="store.shopping.catalog">
      <div className="workspaceCrumbs">
        <button data-action-id="mobi.store.catalog.back" type="button" className="backKey" onClick={onBack}>← Back</button>
      ...
      <div className="catalogToolbar">
        ...
        <button data-action-id="mobi.store.catalog.refresh" type="button" onClick={load}>{tr(lang, "Làm mới", "Refresh", "刷新")}</button>
      </div>
      {loading ? <div className="semanticEmpty">{tr(lang, "Đang tải tổng kho…", "Loading catalog…", "正在加载商品…")}</div> : error ? (
        <div className="semanticEmpty"><b>{tr(lang, "Chưa tải được tổng kho", "Catalog unavailable", "商品目录不可用")}</b><span>{error}</span><button data-action-id="mobi.store.catalog.retry" type="button" onClick={load}>{tr(lang, "Thử lại", "Retry", "重试")}</button></div>
      ) : products.length ? (
