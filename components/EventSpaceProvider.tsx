...
function RuntimeResultToast({result, onClose, onConfirm}: {result: RuntimeResult; onClose: () => void; onConfirm?: () => void}) {
  ...
  return <aside className={`runtimeResultToast state-${result.status}`} role="status" aria-live="polite">
    <button data-action-id="mobi.components.eventspaceprovider.button.001" type="button" className="runtimeToastClose" onClick={onClose} aria-label="Đóng" data-runtime-ignore="true">×</button>

    <b>{result.message}</b>
    {plan && <span>{plan.target} · {plan.fps} FPS · {plan.codecProfile}{plan.enhancedLabel ? ` · ${plan.enhancedLabel}` : ""}</span>}

    {result.status === "needs-confirmation" && onConfirm && <button data-action-id="mobi.components.eventspaceprovider.button.002" type="button" className="runtimeToastConfirm" onClick={onConfirm} data-runtime-ignore="true">Xác nhận</button>}
  </aside>;
}
