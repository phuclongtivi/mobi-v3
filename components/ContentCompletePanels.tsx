...
const Crumbs=({onBack,title}:{onBack:()=>void;title:string})=><div className="workspaceCrumbs"><button data-action-id="mobi.content.back" type="button" className="backKey" onClick={onBack}>← Back</button><span className="crumbKey selected">{title}</span></div>;

export function AppearanceCenter({lang,onBack}:Props){
 const[viewport,setViewport]=useState("auto");
 return <section className="navWorkspace contentSurface" data-runtime-area="me.appearance"><Crumbs onBack={onBack} title={tr(lang,"Giao diện","Appearance","外观")}/><div className="completionPanel"><h3>{tr(lang,"Theme & hiển thị","Theme & display","主题与显示")}</h3><ThemePicker/><div className="completionGrid">{[["auto","Tự động"],["mobile","Mobile"],["desktop","Web · 1920×1080"],["tv","TV · Safe area"]].map(([id,name])=><button data-action-id={`mobi.appearance.viewport.${id}`} key={id} type="button" className={viewport===id?"priority":""} onClick={()=>setViewport(id)}>{name}</button>)}</div><div className="endStatus"><b>✓</b><span>{tr(lang,"Thiết lập hiển thị được lưu trên thiết bị.","Display settings are saved on this device.","显示设置保存在此设备上。")}</span></div></div></section>
}
...
export function SoundCenter({lang,onBack}:Props){
 ...
 return <section className="navWorkspace contentSurface" data-runtime-area="me.sound"><Crumbs onBack={onBack} title={tr(lang,"Âm thanh","Sound","声音")}/><div className="completionPanel"><p className="muted">{tr(lang,"Nhạc nền sản xuất được quản lý như một input của Bàn Mix; màn này quản lý quyền và thiết bị mặc định.","Production music is a Mixer input; this screen manages permissions and defaults.","制作音乐作为调音台输入；此页管理权限和默认设备。")}</p><button data-action-id="mobi.sound.microphone.permission" type="button" className="endCommit" onClick={requestMic}>{tr(lang,"Kiểm tra quyền Micro","Check microphone permission","检查麦克风权限")}</button>{status&&<div className="notice">{status}</div>}</div></section>
}
...
export function PrivacyCenter({lang,onBack}:Props){
 ...
 return <section className="navWorkspace contentSurface" data-runtime-area="me.privacy"><Crumbs onBack={onBack} title={tr(lang,"Quyền riêng tư","Privacy","隐私")}/><div className="completionPanel"><button data-action-id="mobi.privacy.local-first.toggle" type="button" onClick={()=>setLocalFirst(value=>!value)} className={localFirst?"priority":""}>{tr(lang,"Ưu tiên xử lý local","Prefer local processing","优先本地处理")}: {localFirst?"ON":"OFF"}</button><button data-action-id="mobi.privacy.save" type="button" className="endCommit" onClick={save}>{tr(lang,"Lưu quyền riêng tư","Save privacy","保存隐私设置")}</button>{message&&<div className="notice">{message}</div>}</div></section>
}
...
export function StickerStore({lang,onBack,mode}:{lang:Lang;onBack:()=>void;mode:"store"|"wallet"}){
 ...
 return <section className="navWorkspace contentSurface" data-runtime-area="me.stickers"><Crumbs onBack={onBack} title={mode==="store"?"Sticker Store":tr(lang,"Sticker của tôi","My Stickers","我的贴纸")}/><div className="completionPanel"><div className="stickerSummary"><b>{tr(lang,"Điểm Sticker","Sticker Points","贴纸积分")}</b><span>{wallet?.points??"—"}</span></div>{loading?<div className="semanticEmpty">{tr(lang,"Đang tải…","Loading…","加载中…")}</div>:items.length?<div className="stickerGrid">{items.map((item:StickerItem,index:number)=><button data-action-id={`mobi.sticker.${mode}.${item.id||index}.select`} type="button" key={item.id||item.name} onClick={()=>setCount(value=>value+1)}><span className="stickerEmoji">{item.emoji||"✨"}</span><b>{item.name}</b></button>)}</div>:<div className="semanticEmpty">{error||tr(lang,"Chưa có sticker trong kho.","No stickers are available.","暂无贴纸。")}</div>}<div className="endStatus"><b>{count}</b><span>{tr(lang,"Giao dịch chỉ hoàn tất sau receipt Store/Points.","Transactions complete only after a Store/Points receipt.","交易仅在收到商店/积分凭证后完成。")}</span></div></div></section>
}
...
export function PaymentCenter({lang,onBack}:Props){
 ...
 return <section className="navWorkspace contentSurface mode-pay" data-runtime-area="store.payment"><Crumbs onBack={onBack} title={tr(lang,"Phương thức thanh toán","Payment Methods","支付方式")}/><div className="completionPanel"><div className="paymentMethods">{methods.map(([id,name])=><button data-action-id={`mobi.payment.method.${id}.select`} type="button" key={id} className={method===id?"selected priority":""} onClick={()=>setMethod(id)}>{name}</button>)}</div><div className="paymentDetail"><b>{tr(lang,"Đã chọn","Selected","已选择")}: {method}</b><p>{tr(lang,"Chưa tạo giao dịch. Thanh toán chỉ bật khi có giỏ hàng, provider và receipt backend.","No transaction has been created. Payment requires a cart, provider and backend receipt.","尚未创建交易；支付需要购物车、服务商和后端凭证。")}</p></div><button data-action-id={`mobi.payment.method.${method}.checkout`} data-runtime-ignore="gated-until-cart-provider" type="button" className="endCommit" disabled>{tr(lang,"Chưa có provider/giỏ hàng","Provider/cart unavailable","服务商/购物车不可用")}</button></div></section>
}
