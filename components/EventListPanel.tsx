...
export default function EventListPanel({lang,filter}:{lang:Lang;filter:"gift"|"no-gift"|"ticket"}){
 ...
 return <section className="eventListPanel" data-runtime-area={`home.events.${filter}`}><div className="eventListHead"><h2>{title}</h2><button data-action-id={`mobi.events.${filter}.refresh`} type="button" onClick={load}>{tr(lang,"Làm mới","Refresh","刷新")}</button></div>{loading?<div className="semanticEmpty">{tr(lang,"Đang tải sự kiện…","Loading events…","正在加载活动…")}</div>:error?<div className="semanticEmpty"><span>{error}</span><button data-action-id={`mobi.events.${filter}.retry`} type="button" onClick={load}>{tr(lang,"Thử lại","Retry","重试")}</button></div>:rows.length?<div className="eventResultGrid">{rows.map(item=><article key={item.id} className="eventResultCard"><small>{new Date(item.starts_at).toLocaleString()} · {item.event_format}</small><h3>{item.title}</h3><p>{item.description||item.location||""}</p><div><span>{item.has_gift?"🎁 ":""}{item.has_ticket?"🎫 ":""}{item.status}</span><a data-action-id={`mobi.events.${item.id}.open`} href={`/event/${item.public_token}`}>{tr(lang,"Xem sự kiện","Open event","查看活动")}</a></div></article>)}</div>:<div className="semanticEmpty">{tr(lang,"Chưa có sự kiện phù hợp bộ lọc.","No matching events.","没有匹配的活动。")}</div>}</section>
}
