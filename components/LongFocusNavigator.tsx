...
export default function LongFocusNavigator({items,activeId,onSelect,lang}:{items:NavChild[];activeId:string;onSelect:(id:string)=>void;lang:Lang}){
 ...
 return <div className="longFocusNav" aria-label="Long Focus Navigator">
   <button data-action-id="mobi.components.longfocusnavigator.button.001" className="quickKey left" onClick={()=>precision(-1)} onPointerDown={()=>hold(-1)} onPointerUp={stop} onPointerLeave={stop}><b>◀</b><span>QUICK</span><small>tap • double • hold</small></button>
   <div className="focusStage" style={{transform:`translateX(${fineOffset}px)`}}>
    ...
    {visible.map(({item,d})=>{const a=d===0,x=d*132,abs=Math.abs(d),scale=a?1:Math.max(.60,1-abs*.12),rot=d===0?0:(d<0?16+abs*6:-16-abs*6);return <button data-action-id="mobi.components.longfocusnavigator.button.002" key={item.id} className={`focusItem ${a?"active":""}`} onClick={()=>onSelect(item.id)} style={{transform:`translate3d(${x}px,0,${-abs*58}px) rotateY(${rot}deg) scale(${scale})`,opacity:a?1:Math.max(.28,1-abs*.19),zIndex:30-abs}}><span>{label(item.label,lang)}</span>{a&&<em>FOCUS</em>}</button>})}
   </div>
   <button data-action-id="mobi.components.longfocusnavigator.button.003" className="quickKey right" onClick={()=>precision(1)} onPointerDown={()=>hold(1)} onPointerUp={stop} onPointerLeave={stop}><b>▶</b><span>QUICK</span><small>tap • double • hold</small></button>
 </div>
}
