...
export default function EventStructuredCreateForm({lang="vi",onBack}:{lang?:Lang;onBack?:()=>void}){
  ...
  return <section className="navWorkspace contentSurface structuredEventForm" data-runtime-area="home.quickcreate.event.new">
    <div className="workspaceCrumbs">{onBack&&<button data-action-id="mobi.event.form.back" type="button" className="backKey" onClick={onBack}>← Back</button>}<span className="crumbKey selected">{vi?"Sự kiện mới":"New event"}</span></div>
    ...
    <div className="endCommitRow"><button data-action-id="mobi.event.form.create" className="endCommit" disabled={busy||!title.trim()||!start||!end} onClick={create}>{busy?(vi?"Đang tạo…":"Creating…"):(vi?"Tạo Event Space":"Create Event Space")}</button></div>
  </section>
}
