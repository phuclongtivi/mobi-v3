...
export default function EventSpace(){
 ...
 if(!joined)return <main className="page"><section className="eventGate"><span className="eyebrow">EVENT GATE</span><h1>{event.title}</h1><p>Join first, install later.</p><div className="guestJoin"><input value={name} onChange={e=>setName(e.target.value)} placeholder="Tên hiển thị"/><button data-action-id="mobi.app.event.token.page.button.001" className="action" onClick={join}>Tiếp tục vào sự kiện</button><span>Không bắt buộc cài app hoặc đăng nhập. Có thể liên kết tài khoản sau.</span></div>{qr&&<img src={qr} alt="Event QR" style={{width:220,borderRadius:16}}/>}</section></main>;
 ...
 return <main className="page eventSpace">
  ...
  <div className="serviceDock">{services.map(x=><button data-action-id={`mobi.event.service.${x.toLowerCase().replace(/[^a-z0-9]+/g,"-")}`} key={x} className={service===x?"active":""} onClick={()=>setService(x)}>{x}</button>)}</div>
  {service==="Chat room"?<section className="panel"><div className="kpi">Event Chat • {online.length} online</div>
    ...
    <div className="chatComposer"><input value={text} onChange={e=>setText(e.target.value)} onKeyDown={e=>e.key==="Enter"&&sendText()} placeholder="Nhắn trong sự kiện…"/><button data-action-id="mobi.app.event.token.page.button.003" className="action" onClick={sendText}>Gửi</button><input ref={el=>{fileRef.current=el}} type="file" onChange={e=>e.target.files?.[0]&&upload(e.target.files[0])}/></div>
